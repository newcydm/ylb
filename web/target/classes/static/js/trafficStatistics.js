...
$(function () {
	...
	//金额输入框
	$("#bidMoney").keyup(function () {
		var money=$("#bidMoney").val();
		if(money.trim()==''){
			$("#errMessage").text("投资金额不能为空");
			return;
		}
		if(money<=0){
			$("#errMessage").text("投资金额应为大于 0 的整数");
			return;
		}

		if(money%100!=0){
			$("#errMessage").text("投资金额应为 100 的整数");
			return;
		}

		if(parseFloat(money)<$("#bidMinLimit").val()){
			$("#errMessage").text("投资金额不能低于起投金额");
			return;
		}

		if(parseFloat(money)>$("#bidMaxLimit").val()){
			$("#errMessage").text("单笔投资金额不能超过最大投资限额");
			return;
		}
		$("#errMessage").text("");
		money=parseFloat(money)*($("#rate").val()/100/365)*$("#cycle").val()+parseFloat(money);

		$("#success").text('本息：'+money);
	...
	$("#investNow").click(function () {
		$("#bidMoney").keyup()
		if($("#errMessage").text().length>0){
			return;
		}
		if($("#balance").val()<parseFloat($("#bidMoney").val())){
			$("#errMessage").text("余额不足")
			return;
		}

		if($("#productStatus").val()!=0){
			$("#errMessage").text("产品已满标")
			return;
		}
		//验证用户是否实名认证
		...
		$.ajax({
			...
			success:function (result) {
				...
				if(result.code==200){
					//显示投资成功
					$("#dialog-overlay1").prop("style","display:block")
					$("#failurePayment").prop("style","display:block")
					//刷新当前页面
				}else{
